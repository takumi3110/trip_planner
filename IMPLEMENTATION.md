...
## フェーズ 6: デザインのカスタマイズとテーマ機能の実装

アプリのデザインを可愛らしい印象に変更し、ユーザーがテーマカラーをカスタマイズできる機能を追加します。基本のカラーパレットは淡い色使いとします。

- [x] デザインコンセプトの定義: 「可愛らしい感じ」の具体的な要素（フォント、アイコンスタイル、シェイプなど）を決定する。
- [x] カラーパレットの選定: 淡い色使いを基調としたカラーパレットを選定する。
- [x] 設定画面の作成: アプリの設定を管理するための画面を作成する。
- [x] テーマ機能の実装:
    - [x] `ThemeData`をカスタマイズし、選定したカラーパレットを適用する。
    - [x] ユーザーがテーマカラーを選択・変更できるUIを実装する。
    - [x] 選択されたテーマカラーをアプリ全体に適用するロジックを実装する。
- [x] 既存UIへの適用: 既存の画面（旅行プラン作成画面、旅程詳細画面）に新しいデザインとテーマカラーを適用する。
- [x] テストの追加: テーマ機能のユニットテストおよびウィジェットテストを追加する。
